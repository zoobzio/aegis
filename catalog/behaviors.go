...
type SecurityProcessor[T any] pipz.Processor[SecurityInput[T], SecurityOutput[T]]

// ValidationProcessor processes validation behaviors
type ValidationProcessor[T any] pipz.Processor[ValidationInput[T], ValidationOutput[T]]

// DefaultsProcessor generates default values
type DefaultsProcessor[T any] pipz.Processor[T, T]

// ScopeProcessor extracts scope information
type ScopeProcessor[T any] pipz.Processor[ScopeInput[T], ScopeOutput]

// HELPER FUNCTIONS

// GetSecurityPipeline returns the security pipeline for type T
func GetSecurityPipeline[T any]() *pipz.ServiceContract[SecurityBehaviorKey, SecurityInput[T], SecurityOutput[T]] {
	return pipz.GetContract[SecurityBehaviorKey, SecurityInput[T], SecurityOutput[T]]()
}

// GetValidationPipeline returns the validation pipeline for type T
func GetValidationPipeline[T any]() *pipz.ServiceContract[ValidationBehaviorKey, ValidationInput[T], ValidationOutput[T]] {
	return pipz.GetContract[ValidationBehaviorKey, ValidationInput[T], ValidationOutput[T]]()
}

// GetDefaultsPipeline returns the defaults pipeline for type T
func GetDefaultsPipeline[T any]() *pipz.ServiceContract[DefaultsBehaviorKey, T, T] {
	return pipz.GetContract[DefaultsBehaviorKey, T, T]()
}

// GetScopePipeline returns the scope extraction pipeline for type T
func GetScopePipeline[T any]() *pipz.ServiceContract[ScopeBehaviorKey, ScopeInput[T], ScopeOutput] {
	return pipz.GetContract[ScopeBehaviorKey, ScopeInput[T], ScopeOutput]()
}