...
func TestNewNode(t *testing.T) {
	node := NewNode("node-1", "test-node", NodeTypeGeneric, "localhost:8080")
...
func TestNewNodeCustomType(t *testing.T) {
	customType := NodeType("processor")
	node := NewNode("node-1", "test-node", customType, "localhost:8080")

	if node.Type != customType {
		t.Errorf("expected type %s, got %s", customType, node.Type)
	}
}

func TestNodeString(t *testing.T) {
	node := NewNode("node-1", "test-node", NodeTypeGeneric, "127.0.0.1:9090")
	expected := "Node[generic:node-1] test-node @ 127.0.0.1:9090"
...
func TestNodeJSONMarshal(t *testing.T) {
	node := NewNode("node-1", "test-node", NodeTypeGeneric, "10.0.0.1:5432")
	...
	if result["type"] != "generic" {
		t.Errorf("expected type 'generic', got '%v'", result["type"])
	}
...
func TestNodeValidate(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "valid node",
			node:    NewNode("node-1", "test", NodeTypeGeneric, "localhost:8080"),
			wantErr: false,
		},
		{
			name:    "empty ID",
			node:    NewNode("", "test", NodeTypeGeneric, "localhost:8080"),
			wantErr: true,
		},
		{
			name:    "empty name",
			node:    NewNode("node-1", "", NodeTypeGeneric, "localhost:8080"),
			wantErr: true,
		},
		{
			name:    "empty address",
			node:    NewNode("node-1", "test", NodeTypeGeneric, ""),
			wantErr: true,
		},
		{
			name:    "invalid address format",
			node:    NewNode("node-1", "test", NodeTypeGeneric, "not-a-valid-address"),
			wantErr: true,
		},
	}
